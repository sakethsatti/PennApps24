...
const MediaProcessing = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      }>("http://localhost:8000/predict", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      setPredictions(response.data.predictions);
      const setSounds = await fetch("http://localhost:8000/storeSounds", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // <-- Include the Content-Type header
        },
        body: JSON.stringify({
          sound: `${response.data.predictions[0].class}`,
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center p-4 bg-white min-h-screen font-[Outfit]">
      ...
      <p className="text-lg mb-4 text-black text-center">
        Choose an option to begin audio processing.
      </p>
